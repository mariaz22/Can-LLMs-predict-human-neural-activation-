...
class _PytorchTransformerWrapper(BrainModel, TaskModel):
    ...
    def _sentence_features(self, batch):
        ...
        if any(self.identifier.startswith(first_token_model) for first_token_model in
            ...
            return sequence_output[:, 0]  # sentence features from first token (usually CLS)
                 ...
                 ['distilgpt2', 'openaigpt', 'gpt', 'xlnet', 'ctrl', 'transfo-xl-wt103', 't5', 'phi']):
    ...
    class ModelContainer:
        ...
        def __call__(self, sentences, layers):
            ...
            # Check alignment - if mismatch, pad or truncate to match expected size
            expected_words = sum(num_words)
            actual_words = encoded_layers[0].shape[1] if encoded_layers else 0
            if actual_words != expected_words:
                import logging
                logging.warning(f"Token alignment mismatch: expected {expected_words}, got {actual_words}. Adjusting...")
                if actual_words < expected_words:
                    # Pad with zeros
                    padding = expected_words - actual_words
                    encoded_layers = [np.pad(layer, ((0, 0), (0, padding), (0, 0)), mode='constant')
                                      for layer in encoded_layers]
                else:
                    # Truncate
                    encoded_layers = [layer[:, :expected_words, :] for layer in encoded_layers]
        ...
        def align_tokens(self, tokenized_sentences, sentences, max_num_words, additional_tokens, use_special_tokens):
            ...
            for token_index in tqdm(range(len(tokenized_sentences)), desc='token features'):
                ...
                if sentence_index >= len(sentences_chain):
                    break
...
# phi-2 (Microsoft)
transformer_configurations.append(dict(
    prefix='AutoModelForCausalLM', identifier='phi-2', weight_identifier='microsoft/phi-2',
    config_ctr='AutoConfig', model_ctr='AutoModelForCausalLM', tokenizer_ctr='AutoTokenizer',
    tokenizer_special_tokens=('ġ',),
    # Phi-2 has 32 layers
    layers=('embed_tokens',) + tuple(f'model.layers.{i}' for i in range(32))
))
